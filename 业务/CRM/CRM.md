...
为了便于工单系统的扩展，设置不同的工单类型。同时为了支持工单的多任务执行，设置工单任务模型。



### 工单实体设计

- 工单（work order）：系统生成或者人工创建
- 任务（task）：一个工单可以有一个或多个任务。比如审核任务。
- 处理记录（action）：工单或者任务的处理动作，流水记录。分配转让给不同的运营就会新增一条处理记录
