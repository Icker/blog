...
Kafka将自身的元数据信息（metadata）默认存储在zookeeper中。
...
kafka的性能和数据大小无关，长时间存储数据没有什么问题。如下图：

![](https://blog.airaccoon.cn/img/bed/20190621/1561100156044.png)

zookeeper中保存的每一个消费者的唯一的元数据就是偏移量（offset），即消费在分区日志上的位置。

偏移量由消费者控制。

通常一个消费者在读取数据时会线性的增加其偏移量，但实际上，由于位置是由消费者自身控制，因此它可以按照自己喜欢的任何顺序使用记录。例如，消费者可以重置一个老的偏移量以再次处理过去的数据。

一个主题由多个分区组成，每个分区可以分布在不同的主机。因此一个主题可以处理无限量的数据。



## 分布式

分区日志分布在kafka集群的每一个服务器上。

每一个分区都有一个服务器作为**leader**，多个服务器作为**follower**。leader服务器负责该分区数据的读写操作，其他的follower则负责对leader中的数据的复制备份。

当leader宕机了，就会根据一定的选举机制从若干个follower中重新选出一个leader。

每台server都可以成为某些分区的leader和某些分区的follower，因此kafka集群的负载时均衡的。
...
生产者负责指定哪些数据发布到主题中的哪些分区上。

至于如何发布分散到分区上，之后会详细说明。



## 消费者

消费者们通过组名（**consumer group name**）标记他们自身。每一个发布到主题上的消息都会被投递到一个消费者实例中，这个消费者实例是订阅了该主题的消费者组中众多消费者中的一员。

消费者实例可以是单个线程，也可以是个单独的服务器。

1. 如果所有的消费者实例都有相同的消费组，那么他们订阅的每一个消息将会以负载均衡的形式分布在各个消费者中。

2. 如果所有的消费者实例都有不同的消费组，那么他们订阅的每一个消息都会被广播到所有的消费者实例中。

有两个kafka集群，各拥有两个分区，共四个分区，有两个消费组。消费组A有两个消费者实例，B有四个。
...
![](https://blog.airaccoon.cn/img/bed/20190621/1561104398794.png)
